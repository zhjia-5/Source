...
import qs from "querystring";//可以将url地址转为对象
// mock会拦截以"/api/blogtype"开头，方式为get的所有请求，并返回模拟数据
...
Mock.mock("/api/blogtype", "get", {
  code: 0,
  msg: "",
  "data|6-15": [// 随机生成6到15个对象，每个对象如下：
    {
      "id|+1": 1,//id从1开始，每条递增+1
      name: "分类@id",//name为分类+id
      "articleCount|0-200": 0,//0到300条数据，
      "order|+1": 1,//排序，从一开始自增
    },
  ]
});
 ...
 *  (\?.+)? : 表示括号出现0或者1次。
 * 
 * mock：Mock.mock("/api/blog","get",function(options){})
 *  第三个函数可以接受一个函数，该函数的参数options对象
 *    options:{
 *      body: null
 *      type: "GET"
 *      url: "/api/blog?page=1&limit=20&categoryid=-1"
 *    }
 *  
 * querystring插件：
 *  可将url字符串转为对象
...
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function (options) {
  const query = qs.parse(options.url);//可以将url地址转为对象
  return Mock.mock({
    code: 0,
    msg: "",
    data: {// 返回对象
      ...
      [`rows|${query.limit || 10}`]: [
        {
          id: "@guid",//全球唯一ID
          "title|1-5": "@ctitle",//中文标题
          description: "@cparagraph(2, 5)",//中文段落，有5-10句话
          category: {//博客所属分类
            "id|1-10": 0,
            name: "分类@id",
          },
          ...
          //缩略图地址：size,background,foreground,图片文字。缩略图可选，如果没有返回null
          "thumb|1": [Mock.Random.image("300x250", "#373737", "#fff", "Mock Image"), null],
          createDate: `@date('T')`,//时间搓
        },
      ],
    },
  });
...
Mock.mock(/^\/api\/blog\/[^/]+$/, "get", {
  ...
  data: {
    ...
    createDate: "@date('T')",
    toc: [
      { name: "概述", anchor: "article-md-title-1" },//anchor对应 htmlContent里面的h1
      {
        name: "简单请求",
        anchor: "article-md-title-2",//anchor对应 htmlContent里面的h2
        children: [
